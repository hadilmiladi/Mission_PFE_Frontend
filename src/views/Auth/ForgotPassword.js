    ...
    if (Object.keys(inputErrors).length === 0) {
      try {
        ...
        if (error?.response?.status === 409) {
          setErrors({ email: "No user exist with this email" });
        }
      }
    }
    setSpinning(false);
              ...
              {success && (
                <Alert color="success">
                  <div className="alert-body text-center">
                    An email was sent successfullt, Please check your email for
                    further instructions
                  </div>
                </Alert>
              )}
              <Button color="primary" type="submit" block disabled={success}>
                {!spinning ? "Send reset link" : <Spinner size="sm" />}
              </Button>
            </Form>
